...
void game::hello(){
    ...
    erase();
    Grapher.print_vector(1, 1, readFile("./Data/images/a.ansi"));
    mvprintw(4, 4, "My wife is cute <3");
    refresh();
    pause();
    ...
    erase(), box(stdscr, '|', '-');
    Grapher.print_vector(4, 24, DungeonLOGO);
    Grapher.print_string(20,36, "Hello, ", name, ", "); 
...
inline void game::g_status(){
    mvprintw(3, 2, "Status: ");
    Grapher.print_string(5, 6, "Player: ", player->name);
...
inline void game::g_main(){
    ...
    if(cur->adj[U]){
        for(int i = 1; i <= 4; i++) mvprintw(i, stc + 17, "|");
        for(int i = 1; i <= 4; i++) mvprintw(i, scr_col - 18, "|");
    } else {
    ...
    if(cur->adj[D]){
        for(int i = 3; i <= 9; i++) mvprintw(scr_row - i - 1, stc + 17, "|");
        for(int i = 3; i <= 9; i++) mvprintw(scr_row - i - 1, scr_col - 18, "|");
    } else {
        for(int i = stc + 18; i < 111; i++) {
            mvprintw(27, i, "v");
            for(int j = 2; j < 9; j++)
                mvprintw(scr_row - j - 2, i, "\\");
        }
    }
...
inline void game::g_viewBag(int cur){
    for(int i = cur - cur % 20; i < player->bag.size() and i < cur - cur % 20 + 20; i ++){
        Grapher.print_string(10 + i, 64, i == cur?"> ":"  ", player->bag[i].name, (player->get("eq" + to_string(player->bag[i].prop_type)) == Hash(player->bag[i].name))?" (Equiped)":"");
    }
}
