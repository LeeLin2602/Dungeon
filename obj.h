...
public:
	...
	monster(){}
	monster(string n, int h, int a, int d, int r): hp(h), atk(a), dfs(d), reward(r) {
		name = n;
		type = 1;
		valid = 1;
	}
...
public:
	...
	props(){}
	props(string n, vector<string> cmd, int pt, int idx){
		name = n;
		commands = cmd;
		valid = 1;
		type = 2;
...
public:
	...
	npc(){}
	npc(string n, string sc, vector<string> cm, vector<int> nb){
		name = n;
		script = sc;
		commodity = cm;
		number = nb;
